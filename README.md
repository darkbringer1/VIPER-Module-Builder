...
# VIPER Architecture Swift Style Guide.
...
The example above demonstrates the following style guidelines:
 ...
 + Organize extra functionality (e.g. printing) in extensions will help us better debugging.
...
Marking classes or members as `final` in codebase can distract from the main topic and is not required. Nevertheless, use of `final` can sometimes clarify your intent and is worth the cost. In the below example, `Box` has a particular purpose and customization in a derived class is not intended. Marking it `final` makes that clear.
...
Always use Swift's native types and expressions when available. Swift offers bridging to Objective-C so you can still use the full set of methods as needed. Also you should declare type of parameter.
...
let hypotenuse = side * Constants.Math.root2
...
In larger expressions, optional parentheses can sometimes make code read more clearly but its up to you.
